...
import type { Course, Module, Certificate, UserMembership } from "../types";
...
export default function AcademiaSection() {
  ...
  const { userName, userEmail, userMembership, certificates, addCertificate, onChangeProfile } = useAuth();
  const { enrolledCourses, courseProgress, completedModules, favorites, enrollCourse, completeModule } = useProgress();
  ...
  const handleGenerateCertificate = (course: Course) => {
    ...
    addCertificate(cert);
    alert(`¡Felicidades ${userName}! Se ha generado tu certificado oficial para "${course.title}". Puedes consultarlo y descargarlo en la sección "Mis Certificados" más abajo.`);
  };
  ...
  if (!activeCourseId && !showCatalog && !paramCourseId) {
    return (
      <div className="space-y-8 py-4" id="academia-dashboard">
        ...
        {/* Sección de Perfil, Certificados y Favoritos integrada */}
        <div className="border-t border-stone-200 pt-8 space-y-6">
          <div className="flex items-center gap-2">
            <UserIcon className="h-5 w-5 text-primary" />
            <h3 className="font-serif text-lg font-bold text-stone-900">Mi Perfil Estudiantil</h3>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white border border-stone-200 rounded-3xl p-6 space-y-4 border-b-4 border-gold shadow-xs">
              <div className="flex flex-col items-center text-center space-y-3">
                <div className="h-16 w-16 rounded-full bg-primary text-stone-50 border-4 border-stone-100 flex items-center justify-center text-2xl font-serif font-black shadow-inner">
                  {userName.charAt(0)}
                </div>
                <div>
                  <h4 className="font-serif text-base font-bold text-stone-900">{userName}</h4>
                  <p className="text-xs text-stone-500 font-mono mt-0.5">{userEmail}</p>
                </div>
                <div className="inline-flex items-center gap-1.5 px-3 py-1 text-[10px] font-mono tracking-wider font-bold rounded-full border bg-gold/10 text-stone-950 border-gold/40">
              ...
              {editMode ? (
                <form onSubmit={handleSaveProfile} className="space-y-3 border-t border-stone-200 pt-4">
                  <div className="space-y-1">
                    <label className="text-[9px] font-mono text-stone-500 uppercase">Nombre</label>
                    <input type="text" required value={tempName} onChange={e => setTempName(e.target.value)}
                      className="w-full rounded-xl bg-stone-50 border border-stone-200 px-3 py-2 text-xs outline-none focus:border-gold" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-mono text-stone-500 uppercase">Correo</label>
                    <input type="email" required value={tempEmail} onChange={e => setTempEmail(e.target.value)}
                      className="w-full rounded-xl bg-stone-50 border border-stone-200 px-3 py-2 text-xs outline-none focus:border-gold" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-mono text-stone-500 uppercase">Membresía</label>
                    <select value={tempMembership} onChange={e => setTempMembership(e.target.value as UserMembership)}
                      className="w-full rounded-xl bg-stone-50 border border-stone-200 px-3 py-2 text-xs outline-none focus:border-gold">
                  ...
                  <div className="flex gap-2 pt-1">
                    <button type="button" onClick={() => setEditMode(false)}
                      className="flex-1 py-2 bg-stone-100 text-stone-700 text-xs font-semibold rounded-xl border border-stone-200">Cancelar</button>
                    <button type="submit"
                      className="flex-1 py-2 bg-primary text-stone-50 text-xs font-semibold rounded-xl hover:bg-gold hover:text-stone-950 transition-all">Guardar</button>
                  </div>
                </form>
              ) : (
                <div className="border-t border-stone-200 pt-4">
                  <button onClick={() => { setTempName(userName); setTempEmail(userEmail); setTempMembership(userMembership); setEditMode(true); }}
                    className="w-full py-2.5 bg-primary hover:bg-gold hover:text-stone-950 text-stone-50 text-xs font-semibold rounded-xl transition-all inline-flex items-center justify-center gap-1.5">
            ...
            <div className="bg-white border border-stone-200 rounded-3xl p-6 space-y-4 border-b-4 border-gold shadow-xs">
              <h4 className="font-serif text-sm font-bold text-stone-900 flex items-center gap-2">
                ...
                <span>Certificados ({displayCerts.length})</span>
              </h4>
              {displayCerts.length > 0 ? (
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {displayCerts.map(cert => (
                    <div key={cert.id} onClick={() => setActiveCert(cert)}
                      className="p-3 bg-stone-50 border border-stone-200 rounded-xl cursor-pointer hover:border-gold transition-all group">
                      <div className="flex justify-between items-center text-[8px] font-mono text-stone-400">
                        <span>{cert.code}</span>
                        <Award className="h-3 w-3 text-gold" />
                      </div>
                      <h5 className="font-serif text-[11px] font-bold text-stone-900 group-hover:text-primary transition-colors line-clamp-2 mt-1">{cert.courseTitle}</h5>
                      <div className="flex items-center justify-between text-[9px] font-mono text-stone-400 mt-1">
                        <span>{cert.date}</span>
                        <span className="text-primary font-bold group-hover:underline">Ver</span>
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="p-4 text-center border border-dashed border-stone-200 rounded-2xl bg-stone-50">
                  <GraduationCap className="h-6 w-6 text-stone-400 mx-auto" />
                  <p className="text-[11px] text-stone-500 mt-1">Completa cursos para obtener certificados.</p>
                </div>
              )}
            </div>

            {/* Favorites + Preferences Card */}
            <div className="bg-white border border-stone-200 rounded-3xl p-6 space-y-4 border-b-4 border-gold shadow-xs">
              <h4 className="font-serif text-sm font-bold text-stone-900 flex items-center gap-2">
                <Heart className="h-5 w-5 text-gold" />
                <span>Lecturas Favoritas ({favoriteDocs.length})</span>
              </h4>
              {favoriteDocs.length > 0 ? (
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {favoriteDocs.map(doc => (
                    <div key={doc.id} onClick={() => navigate("/biblioteca/" + doc.id)}
                      className="p-3 bg-stone-50 border border-stone-100 hover:border-gold rounded-xl cursor-pointer transition-all flex items-center justify-between group">
                      <div className="space-y-0.5 pr-2">
                        <span className="text-[7px] font-mono text-primary bg-gold/10 px-1.5 py-0.5 rounded uppercase font-bold">{doc.category}</span>
                        <h5 className="font-serif text-[11px] font-bold text-stone-900 group-hover:text-primary line-clamp-1">{doc.title}</h5>
                      </div>
                ...
                <div className="p-4 text-center border border-dashed border-stone-200 rounded-2xl bg-stone-50">
                  ...
                  <p className="text-[11px] text-stone-500 mt-1">Marca documentos como favoritos en Biblioteca.</p>
                </div>
              )}
              <div className="border-t border-stone-200 pt-4 mt-2">
                <div className="flex items-center justify-between gap-3 p-3 bg-stone-50 rounded-xl border border-stone-200/50">
                  <div className="space-y-0.5">
                    <p className="text-[11px] font-semibold text-stone-900 flex items-center gap-1.5"><Settings className="h-3.5 w-3.5 text-primary" /> Ahorro de Datos</p>
                    <p className="text-[9px] text-stone-500">Desactiva animaciones pesadas</p>
                  </div>
                  <button onClick={() => toggleDataSaver(!dataSaver)}
                    className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors ${dataSaver ? "bg-primary" : "bg-stone-200"}`} type="button">
                    <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm ring-0 transition ${dataSaver ? "translate-x-4" : "translate-x-0"}`} />
                  </button>
                </div>
              </div>
            </div>
        ...
        {activeCert && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/60 p-4 sm:p-6 backdrop-blur-xs overflow-y-auto">
            <div className="relative w-full max-w-xl rounded-3xl bg-white border border-stone-300 shadow-2xl p-6 sm:p-10 space-y-6 my-8 text-center border-t-8 border-gold">
              <button onClick={() => setActiveCert(null)}
                className="absolute top-5 right-5 text-stone-400 hover:text-stone-700 text-xs font-mono font-bold">Cerrar [X]</button>
              <div className="border-8 border-primary/10 rounded-2xl p-6 sm:p-10 bg-stone-50 relative space-y-6 shadow-xs border-double">
                ...
                <div className="space-y-4">
                  ...
                  <p className="font-serif text-sm sm:text-lg font-bold text-primary leading-snug">&ldquo;{activeCert.courseTitle}&rdquo;</p>
                </div>
              ...
              <div className="flex flex-col gap-2 items-center justify-center">
                <button disabled={isDownloading} onClick={() => { setIsDownloading(true); setTimeout(() => setIsDownloading(false), 2500); }}
                  className="px-5 py-2.5 bg-primary hover:bg-gold hover:text-stone-950 disabled:opacity-50 text-stone-50 text-xs font-bold rounded-xl flex items-center gap-1.5 shadow-sm transition-all uppercase tracking-wide">
                  <Download className="h-4 w-4" />
                  <span>{isDownloading ? "Generando PDF..." : "Imprimir / Descargar Certificado"}</span>
                </button>
                {isDownloading && <div className="text-[10px] text-primary font-mono animate-pulse font-bold">✓ Descarga iniciada.</div>}
              </div>
            </div>
          </div>
        )}

  ...
  if (!course || !nodeInfo) {
    return (
      <div className="p-6 text-center">
        ...
        {resourceId && (
          <button
            onClick={() => navigate(`/campus/${resourceId}`)}
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex gap-2">
        <button
          onClick={handleLeer}
          className="flex-1 py-2.5 bg-stone-800 hover:bg-stone-700 text-white text-xs font-semibold rounded-xl transition-colors inline-flex items-center justify-center gap-1"
        >
...
function RecipeView({ resourceId }: { resourceId?: string }) {
  const navigate = useNavigate();
...
function ResourceListView({ params }: { params: Record<string, string> }) {
  const navigate = useNavigate();
  const results = useMemo(() => {
    if (!params.query) return [];
    return searchNodes(params.query).slice(0, 5);
  }, [params.query]);
      ...
      {results.map((node) => {
        ...
        return (
          <button
            key={node.id}
            onClick={() => {
              const route = isCourse ? '/campus' : isDoc ? '/biblioteca' : '/recursos';
              navigate(route);
            }}
          ...
          >
            <div className="flex items-start gap-2.5">
              ...
              <div className="min-w-0">
                <h5 className="text-xs font-bold font-serif text-stone-900 line-clamp-1 group-hover:text-emerald-700 transition-colors">
                  {node.title}
                </h5>
...
function NodeDetailView({ resourceId }: { resourceId?: string }) {
  const navigate = useNavigate();
  const node = useMemo(() => {
    if (!resourceId) return null;
    return getNode(resourceId);
...
export default function ResourceLayer({ layer }: { layer: Layer }) {
  switch (layer.component) {
    case 'course':
      return <CourseView resourceId={layer.resourceId} />;
    case 'document':
      return <DocumentView resourceId={layer.resourceId} />;
    case 'recipe':
      return <RecipeView resourceId={layer.resourceId} />;
    case 'node':
      return <NodeDetailView resourceId={layer.resourceId} />;
    case 'resource':
      return <ResourceListView params={layer.params} />;
    default:
      return (
        <div className="p-6 text-center">
          <p className="text-xs text-stone-500">Tipo de recurso no disponible</p>
        </div>
      );
  }
}
